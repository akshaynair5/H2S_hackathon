// ================================
// content.js (Unified Handling with Draggable + Auto-Hide)
// ================================
...
header.style.cursor = "grab"; // draggable cursor
...
function setResult(score, explanation) {
  ...
  document.getElementById("text-result").textContent = explanation || "No explanation returned.";
...
function analyzeTextNow() {
  ...
  if (!visibleText) { setError("No visible text found."); return; }

  chrome.runtime.sendMessage(
    { type: "ANALYZE_TEXT", payload: { text: visibleText, url: location.href } },
    (response) => {
      if (!response || response.error) { setError(response?.error || "No response from backend."); return; }
...
function analyzeImagesNow() {
  ...
  if (visibleImages.length === 0) { container.textContent = "No images found on this page."; return; }

  chrome.runtime.sendMessage(
    { type: "ANALYZE_IMAGE", payload: { urls: visibleImages } },
    (response) => {
      container.innerHTML = "";
      if (!response || response.error) { container.textContent = "Image analysis failed or not supported."; return; }
...
chrome.runtime.onMessage.addListener((message) => {
  ...
  switch (message.type) {
    ...
    case "IMAGE_ANALYSIS_RESULT": {
      ...
      if (panel && badge) { badge.style.display = "none"; panel.style.display = "flex"; setTimeout(() => { panel.style.opacity = "1"; panel.style.transform = "translateY(0)"; }, 10); }

      const container = document.getElementById("image-results");
      if (container.textContent === "No images analyzed." || container.textContent === "No images found on this page.") container.innerHTML = "";
      ...
      imgEntry.innerHTML = `<img src="${url}" style="max-width:80px; max-height:50px; margin-right:12px; border-radius:4px; object-fit:cover;"><span>Validity: <strong>${validity}%</strong></span>`;
      if (validity >= 75) imgEntry.style.color = "#0b8043";
      else if (validity >= 45) imgEntry.style.color = "#e09b00";
      else imgEntry.style.color = "#c42f2f";
...
document.addEventListener("mouseup", () => {
  ...
  if (rect.right > window.innerWidth) panel.style.left = window.innerWidth - panel.offsetWidth / 4 + "px";
  else if (rect.left < 0) panel.style.left = -panel.offsetWidth / 4 + "px";
});
